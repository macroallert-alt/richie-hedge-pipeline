...
# ─────────────────────────────────────────────
# GOOGLE SHEETS + FRED CONNECTION (gspread + retry)
# ─────────────────────────────────────────────
...
def open_warehouse_with_retry(client, max_retries=3):
    """Open warehouse sheet with retry on 429 rate limit."""
    for attempt in range(max_retries):
        try:
            ...
            if "429" in str(e) or "Quota exceeded" in str(e):
                wait = 60 * (attempt + 1)
                log.warning(f"Rate limit opening warehouse, waiting {wait}s (attempt {attempt+1}/{max_retries})")
                time.sleep(wait)
            else:
                raise
    raise RuntimeError(f"Failed to open warehouse after {max_retries} retries")


def sheets_op_with_retry(operation, description="", max_retries=3):
    """Execute a gspread operation with retry on 429 rate limit."""
    for attempt in range(max_retries):
        try:
            return operation()
        except gspread.exceptions.APIError as e:
            if "429" in str(e) or "Quota exceeded" in str(e):
                wait = 60 * (attempt + 1)
                log.warning(f"Rate limit on {description}, waiting {wait}s (attempt {attempt+1}/{max_retries})")
                time.sleep(wait)
            else:
                raise
    raise RuntimeError(f"Failed {description} after {max_retries} retries")
...
# ─────────────────────────────────────────────
# GOOGLE SHEETS WRITE (gspread + retry)
# ─────────────────────────────────────────────
...
def write_raw_market(ws_raw_market, today: date, raw_rows: dict):
    ...
    for sheet_key, row_num in RAW_MARKET_ROWS.items():
        ...
        try:
            existing_row = sheets_op_with_retry(
                lambda rn=row_num: ws_raw_market.row_values(rn),
                f"read RAW_MARKET row {row_num}"
            )
        ...
        sheets_op_with_retry(
            lambda rn=row_num, rd=row_data: ws_raw_market.update(
                range_name=f"A{rn}:J{rn}",
                values=[rd],
            ),
            f"write RAW_MARKET row {row_num}"
        )
        log.info(f"RAW_MARKET Row {row_num} written: {sheet_key} = {val_str}")
        time.sleep(2)  # 2s between writes to avoid rate limit
...
def write_scores(ws_scores, today: date, composite: dict):
    ...
    sheets_op_with_retry(
        lambda: ws_scores.update(
            range_name=f"A{SCORES_ROW}:M{SCORES_ROW}",
            values=[row_data],
        ),
        f"write SCORES row {SCORES_ROW}"
    )
    log.info(
...
def write_dashboard(ws_dashboard, today: date, composite: dict):
    ...
    sheets_op_with_retry(
        lambda: ws_dashboard.update(
            range_name=f"A{DASHBOARD_ROW}:H{DASHBOARD_ROW}",
            values=[row_data],
        ),
        f"write DASHBOARD row {DASHBOARD_ROW}"
    )
    log.info(
...
def main():
    ...
    try:
        # Pre-sleep: let parallel collectors finish their burst
        log.info("Waiting 15s before Sheets writes (rate limit protection)...")
        time.sleep(15)

        client    = get_gspread_client()
        warehouse = open_warehouse_with_retry(client)
